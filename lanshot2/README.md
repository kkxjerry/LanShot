...
程序不会自行开始监听。统一入口打开后保持“尚未采集”；只有点击“开始采集”才会录音。点击“停止采集”只停止并保存。按 `F23` 会尽快停止当前识别，把系统声音和麦克风两份文字一起提交给大模型，并把回答写入 `answer.txt`；回答完成后自动开始下一轮采集。主控制进程不会退出，悬浮窗异常关闭时会自动恢复；只有“全部停止”才结束语音模式。开始或停止下一轮不会清空屏幕上的上一份答案、每轮归档或持续对话历史。API Key 从环境变量或 macOS 钥匙串读取，不写入项目和日志。
