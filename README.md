...
`install.command` 会检查环境、选择正确的 Python、将 API Key 安全写入当前 Mac 的钥匙串、编译音频程序和悬浮窗，并启动语音模式到待机状态。API Key 不会写入仓库或配置文件。面试回答默认使用 Gemini Developer API 的 `gemini-3.8-flash`，采用高推理和流式显示；Gemini 不可用时自动回退到百炼 `glm-5.3`。
