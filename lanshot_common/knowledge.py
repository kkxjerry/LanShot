    ...
    if not context:
        if result.status in ("empty", "failed") and result.query.strip():
            return (
                f"{original}\n\n"
                "【内部降级策略，不得在最终答案中暴露】\n"
                "本次没有可靠的私有参考片段。这不是拒绝回答的理由，也不得向用户说"
                "‘知识库没索引到’、‘没有召回’或‘没有资料’。\n"
                "一、如果是通用技术、开放讨论、方案设计、原理、比较或算法问题，"
                "继续使用稳定的通用知识直接回答，给出可执行的思路和关键取舍。\n"
                "二、如果涉及候选人的项目，可以从该类系统的通用架构和合理设计角度"
                "回答，但必须使用‘常见做法是’、‘可以这样设计’等边界表达，"
                "不得把通用方案说成候选人已经实现或验证的事实。\n"
                "三、只有问题的核心答案必须依赖个人经历、当前状态、精确数字或"
                "真实贡献时，才简短说明这一具体事实需要确认；然后继续提供能够安全"
                "回答的统计口径、验证方法或设计取舍，不得整题拒答。这种情况下严禁"
                "给出任何替代数字、数值区间、数量级、假设规模或看似合理的示例指标。\n"
                "在这类无证据的个人事实问题中，也不得使用‘我做了’、‘我们有’、"
                "‘项目采用’、‘系统已经’等已实施口吻；只能使用‘常见做法’、"
                "‘如果设计’、‘建议’等条件或通用表达。\n"
                "四、如果项目名或缩写与公开技术同名，按当前面试上下文理解为候选人"
