...
- Gemini `MEDIUM` 连续 7 次对照中仅 4 次成功，成功样本首段中位数约 11.2 秒、完整中位数约 12.5 秒，两次等待 20 秒后失败。正式默认因此切回百炼 `glm-5.3` 低推理，不再先请求 Gemini。
- 面试模式快捷键改为 `F22` 发送问题、`F23` 答案向上翻半页、`F24` 向下翻半页；截屏模式继续保持相同的 F22/F23/F24 职责。
- 修复语音悬浮窗宽度/高度设置被固定 82%/78% 屏幕尺寸覆盖的问题；语音和截屏模式现使用独立尺寸配置，滑块范围根据实际显示器计算。
- 面试回答使用 Gemini Developer API 的 `gemini-3.8-flash` 中等推理和 SSE 流式输出；悬浮窗每 0.35 秒读取追加内容，自动使用本地 7890 代理，Gemini 不可用时保留 GLM-5.3 降级链路。真实面试样本的高推理首段曾达 18.7 秒，因此默认降为中等推理。
...
- 空召回改为分层降级：通用/开放/设计题继续正常回答，项目题给出明确标记的通用方案，仅对个人经历和精确数字保留；最终答案禁止暴露“没有索引/召回”等内部状态。
- RAG 查询改为优先使用面试官声道，删除测试开场白、“第几题”和麦克风重复/口头语；常见 `codah` 误识别统一为 `CODA PaiCLI`，但本地存档仍保留原始双路转写。
