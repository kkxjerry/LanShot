...
- 语音模式的首次开始与手动停止使用界面按钮；`F23` 专门负责立即结束当前识别，把两路文字一起提交，并在回答后自动开始下一轮。
...
程序不会自动开始第一次语音监听。切换到语音模式只显示待机界面，必须明确点击“开始采集”。点击“停止采集”只保存本轮；按 `F23` 才发送问题并在回答后自动恢复采集。切换到截屏模式或全部停止时，音频采集立即结束。
