...
LanShot 是一个 macOS 本地面试辅助工具，包含截屏模式和双路语音模式。语音模式分别采集系统声音与麦克风，使用千问实时 ASR 转写；按 `F22` 后停止本轮识别，将两路文字和最近对话历史发送给回答模型，并在不可捕获的悬浮窗中流式显示答案。`F23`/`F24` 用于答案向上/向下翻页。两个模式都可选择先检索阿里云百炼知识库，再将召回资料交给现有回答模型。
...
- 可用的阿里云百炼 API Key。
- 可选的 Gemini Developer API Key；未配置时面试回答自动使用 GLM 备用模型。
...
`install.command` 会检查环境、选择正确的 Python、将 API Key 安全写入当前 Mac 的钥匙串、编译音频程序和悬浮窗，并启动语音模式到待机状态。API Key 不会写入仓库或配置文件。面试回答默认使用 Gemini Developer API 的 `gemini-3.8-flash`，采用中等推理和流式显示；Gemini 不可用时自动回退到百炼 `glm-5.3`。

首次点击“开始采集”时，需要在 macOS“隐私与安全性”中允许：

- `LanShot Voice Capture.app`：麦克风、录屏与系统录音。
- 当前 Python 或终端：输入监控，用于接收 `F22`、`F23` 和 `F24`。
...
3. 按 `F22`：尽快停止识别，将系统声音、麦克风文字和最近历史发送给模型。
4. 按 `F23` / `F24`：将当前答案向上/向下滚动半页。
5. 本轮快照保存后立即开始下一轮采集，LLM 在后台并行生成答案。
6. 主控制程序与悬浮窗保持运行；只有执行“全部停止”才会退出。
...
- 未配置、无召回、OCR 失败、超时或百炼错误时，自动回退到原有模型回答，不让发送流程卡住。
...
- 菜单栏麦克风图标中的“退出 LanShot”：保存当前记录并停止采集、悬浮窗、快捷键和控制器。
