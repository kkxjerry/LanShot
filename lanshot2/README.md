...
- 语音模式下 `F23` 与“开始采集 / 停止采集”按钮作用相同；第二次按下会停止本轮采集、提交面试官文字并在答案区显示大模型回答。
...
程序不会自行开始监听。统一入口打开后保持“尚未采集”；点击“开始采集”或按 `F23` 才会录音。再次按 `F23` 或点击“停止采集”会写完音频，将本轮 `interviewer.txt` 提交给大模型，并把回答写入 `answer.txt`。API Key 从环境变量或 macOS 钥匙串读取，不写入项目和日志。
