...
final class MicrophoneRecorder: @unchecked Sendable {
    ...
    init(outputURL: URL, transcriber: LocalTranscriber? = nil) {
        self.outputURL = outputURL
        self.transcriber = transcriber
        try? FileManager.default.removeItem(at: outputURL)
    }
...
struct LanShotAudioCapture {
    static func main() async {
        ...
        do {
            ...
            microphone.stop()
            await interviewerTranscriber.finish()
            await systemWriter.finish()
            try "transcribing\n".write(to: statusURL, atomically: true, encoding: .utf8)
            try? await Task.sleep(for: .milliseconds(500))
            let meAudioURL = outputDirectory.appendingPathComponent("me.wav")
            let meTranscriptURL = outputDirectory.appendingPathComponent("me.txt")
            do {
                ...
                try? "".write(to: meTranscriptURL, atomically: true, encoding: .utf8)
                try? "\(error.localizedDescription)\n".write(
                    to: outputDirectory.appendingPathComponent("me.errors.log"),
                    atomically: true,
                    encoding: .utf8
                )
            }
