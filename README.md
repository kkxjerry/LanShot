...
LanShot 是一个 macOS 本地面试辅助工具，包含截屏模式和双路语音模式。语音模式分别采集系统声音与麦克风，使用千问实时 ASR 转写；按 `F22` 后停止本轮识别，将两路文字和最近对话历史发送给回答模型，并在不可捕获的悬浮窗中显示答案。`F23`/`F24` 用于答案向上/向下翻页。两个模式都可选择先检索阿里云百炼知识库，再将召回资料交给现有回答模型。
